...
export main_dir=/volume/kohan/immunotherapy/MHCfovea

python3 build_dataset.py \
--ms_data ${main_dir}/data/IEDB/mhc_ligand_table_export_1593661050.csv \
--assay_data ${main_dir}/data/IEDB/bdata.20130222.mhci.txt \
--uniprot ${main_dir}/data/Uniprot/uniprot_seq.json \
--encoding_method onehot \
--train_decoy_times 10 \
--test_decoy_times 2 \
--output_dir ${main_dir}/data/raw
...
do
    python3 trainer.py \
    --mhc_encode_file ${main_dir}/data/MHCI/MHCI_res182_onehot.npy \
    --dataframe_dir ${main_dir}/data/raw/dataframe \
    --dataset_dir ${main_dir}/data/raw/onehot \
    ...
    --output_dir ${main_dir}/result/res182_decoy${factor}_CNN_1_1
done
...
--mhc_file ${main_dir}/data/MHCI/MHCI_res182_seq.json \
--peptide_dataframe ${main_dir}/data/raw/dataframe/${data_name}.csv \
--peptide_dataset ${main_dir}/data/raw/onehot/${data_name}.pt \
--encoding_method onehot \
--model_file model/res182_CNN_1.py \
--model_state_dir ${main_dir}/result/${model_state_dir}/model_state \
--model_num ${model_num} \
--output_dir ${main_dir}/prediction/${data_name}/${model_state_dir}_${model_num} \
...
--input_df ${main_dir}/data/raw/dataframe/test.csv \
--output_df ${main_dir}/prediction/test/netmhcpan/prediction.csv \
--pre_run_dir ${main_dir}/prediction/test/netmhcpan/pre_run/ \
--post_run_dir ${main_dir}/prediction/test/netmhcpan/post_run/ \
--num_per_file 1000 \
--function SplitFile \
...
--input_file ${main_dir}/data/raw/dataframe/test.csv \
--output_file ${main_dir}/prediction/test/mhcflurry/prediction.csv \
...
--input_df ${main_dir}/data/raw/dataframe/test.csv \
--output_df ${main_dir}/prediction/test/mixmhcpred/prediction.csv \
--pre_run_dir ${main_dir}/prediction/test/mixmhcpred/pre_run/ \
--post_run_dir ${main_dir}/prediction/test/mixmhcpred/post_run/ \
--function SplitFile \
...
do
    python3 cam_run.py \
    --mhc_file ${main_dir}/data/MHCI/MHCI_res182_onehot.npy \
    --dataframe_file ${main_dir}/prediction/${data_name}/${model_state_dir}_${model_num}/tmp_prediction.csv \
    --dataset_file ${main_dir}/data/raw/onehot/${data_name}.pt \
    --predict_col decoy_${i} \
    --predict_threshold 0.9 \
    --encode onehot \
    --model_path model/res182_CNN_1.py \
    --model_state_path ${main_dir}/result/${model_state_dir}/model_state/decoy_${i}.tar \
    --mhc_target_layers 2 \
    --pre_mhc_len 182 \
    --pre_epitope_len 15 \
    --post_mhc_len 182 \
    --cam_list ScoreCAM \
    --outdir ${main_dir}/cam_result/${model_state_dir}_${data_name}/decoy_${i}
...
export main_dir=/volume/kohan/immunotherapy/MHCfovea
export pred_dir="${main_dir}/prediction/pan_allele"
export mhc_alleles_file="${pred_dir}/selected_alleles.json"
export output_dir="${pred_dir}/select_output"
...
do
    ...
    --encoding_method onehot \
    --model_file model/res182_CNN_1.py \
    --model_state_dir ${main_dir}/result/res182_decoy5_CNN_1_1/model_state \
    --output_dir ${output_dir}/$allele_group \
    --seqlogo_threshold 0.9 \
    --alleles ${alleles}
done
...
export main_dir=/volume/kohan/immunotherapy/MHCfovea
export pred_dir="${main_dir}/prediction/pan_allele"
export mhc_alleles_file="${pred_dir}/last_alleles.json"
export output_dir="${pred_dir}/last_output"
